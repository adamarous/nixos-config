...

{
  ...
  # Eye-candy for the boot process; does nothing for now.
  ...
  programs.system-config-printer.enable = true; # GUI.
  ...
  services.pipewire = {
    enable = true;
    audio.enable = true; # Enable Pipewire as the primary sound server.
    alsa.enable = true;
    jack.enable = true;
    pulse.enable = true;
  };
  ...
  programs = {
	...
	augroup remember_cursor_position
	  autocmd!
	  autocmd BufLeave * if !&buftype =~ 'nofile' |
	    \ let b:cursor_position = getpos(".") |
	    \ endif
